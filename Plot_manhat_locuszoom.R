...
######################
gwasUniq_df1=fread(args[1], sep = '\t', header = T) %>% dplyr::rename(CHROM = CHR, P = pvalue) %>% dplyr::mutate(CHROM = gsub('chr|H', '', CHROM)
gwasUniq_df2=fread(args[2], sep = '\t', header = T) %>% dplyr::rename(CHROM = CHR, P = pvalue) %>% dplyr::mutate(CHROM = gsub('chr|H', '', CHROM)
...
EXPAND=args[5] # on what number expand provided region
if(length(args) > 5){
	KEEP_GENES=args[6] # comma separated list of gene symbols (descriptions) to keep on the region plot OR could be equal NA to not show any gene names, if argument is not specified all gene names will be shown
